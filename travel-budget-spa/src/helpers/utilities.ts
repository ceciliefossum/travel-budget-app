...
const getSummarizedAmount = (
	...
	return transactions
		...
		.reduce(
			(prevAmount: number, currentTransaction: ITransaction) =>
				currentTransaction.amount + prevAmount,
			0
		);
};
...
export const getDailyBudget = (
	...
	const totalIncome = getSummarizedAmount(transactions, budgetPeriod.startDate, new Date(), [
		TransactionType.Income
	]);
	const totalExpenses = getSummarizedAmount(transactions, budgetPeriod.startDate, yesterday, [
		TransactionType.DailyExpence
	]);
...
export const getTodaysBalance = (transactions: ITransaction[], budget: number): number => {
	...
	const total =
		budget +
		getSummarizedAmount(transactions, startOfDay, endOfDay, [TransactionType.DailyExpence]);
	return getRoundedNumber(total);
};
