...
test('calculates account balance', () => {
	const accountBalance = utilities.getAccountBalance(
		utilitiesMock.transactionsMockAccountBalance50
	);
	...
	const accountBalanceNegativeFifty = utilities.getAccountBalance(
		utilitiesMock.transactionsMockAccountBalanceNegative50
	);
	expect(accountBalanceNegativeFifty).toBe(-50);

	const accountBalanceWithFuelExpences = utilities.getAccountBalance(
		utilitiesMock.transactionsMockAccountBalanceWithFuelExpence50
	);
	expect(accountBalanceWithFuelExpences).toBe(700);
});

test('calculates daily budget', () => {
	const dailyBudget = utilities.getDailyBudget(
		utilitiesMock.transactionsMockDailyBudget250,
		utilitiesMock.budgetPeriodDailyBudgetFourDaysTwoDaysLeft
	);
...
test('calculates todays balance', () => {
	const dailyBudget = utilities.getDailyBudget(
		utilitiesMock.transactionsMockTodaysBalance150,
		utilitiesMock.budgetPeriodDailyBudgetFourDaysTwoDaysLeft
	);
	const todaysBalance = utilities.getTodaysBalance(
		utilitiesMock.transactionsMockTodaysBalance150,
		dailyBudget
	);
